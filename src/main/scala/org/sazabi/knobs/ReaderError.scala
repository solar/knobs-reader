...
case class NoSuchKey(key: String) extends ReaderError(s"""Key "${key}" was not found""")

case class UnexpectedType(key: String) extends ReaderError(s"""Key "${key}" has unexpected type""")
